...
func (l *LayerLoader) Load(key string) (*layer.Item, error) {
...
func (l *LayerLoader) LoadThunk(key string) func() (*layer.Item, error) {
	l.mu.Lock()
...
func (l *LayerLoader) LoadAll(keys []string) ([]*layer.Item, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *LayerLoader) LoadAllThunk(keys []string) func() ([]*layer.Item, []error) {
	results := make([]func() (*layer.Item, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *LayerLoader) Prime(key string, value *layer.Item) bool {
	l.mu.Lock()
	var found bool
...
func (l *LayerLoader) Clear(key string) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *LayerLoader) unsafeSet(key string, value *layer.Item) {
	if l.cache == nil {
		l.cache = map[string]*layer.Item{}
	}
	l.cache[key] = value
}
...
func (b *layerLoaderBatch) keyIndex(l *LayerLoader, key string) int {
